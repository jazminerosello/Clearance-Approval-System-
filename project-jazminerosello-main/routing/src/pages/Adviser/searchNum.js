...
useEffect(() => {
  fetch("http://localhost:3001/search-adviser-id?id="+ adviserMail)
    ...
    useEffect(() => {
        fetch("http://localhost:3001/search-student-number?studentNumber="+queryString.parse(window.location.search).studentNumber+ "&adviser=" + queryString.parse(window.location.search).adviser)
          .then(response => response.json())
          .then(body => {
            setStudent(body)
          })
        ...
        <div className="wrapper">
            <div className="main">
                ...
                <div className="approvers" id="approvers-main">
                    <h1>Search Results for {queryString.parse(window.location.search).studentNumber}!</h1>
                    ...
                    <div> 
                        ...
                        <div>
                            <p>{queryString.parse(window.location.search).studentNumber} does not exist in records. </p>
                        </div>
                        )}
                    </div>
